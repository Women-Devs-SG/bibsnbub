...
export default function middleware(
  ...
  // Allow API and tRPC routes to pass through untouched so that
  // Pages Router (`src/pages/api/*`) and App Router API routes work correctly.
  const apiOrTrpc = request.nextUrl.pathname.startsWith('/api') || request.nextUrl.pathname.startsWith('/trpc');
  if (apiOrTrpc) {
